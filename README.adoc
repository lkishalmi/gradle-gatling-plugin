...
Plugin versioning::
Since `Gatling` v3.0.0 this plugin's major version is going to follow `Gatling` 's
major version for making things easier.
  
Source code layout::
Since `Gatling` v3.0.0 and corresponding plugin version `v3.x.x`
there were breaking changes introduced for source code layout. Check corresponding section for details.
