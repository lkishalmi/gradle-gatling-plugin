...
|toolVersion        |String         |'{gatlxingToolVersion}'                     |`Gatling` version
...
|simulationLogFolder        |FILE         |`null`                     |Full path of the folder that contains the `.log` files
|===
...
Defining where are your `.log` files::
+
[source,groovy,subs="attributes"]
----
gatling {
    simulationLogFolder = new File("/home/user/simulationLogFolder/")
}
----
...
|`gatlingGenerateReport`
|GatlingGenerateReportTask
|Generate the report given a folder (specified in `simulationLogFolder`) witch contains `.log` files
